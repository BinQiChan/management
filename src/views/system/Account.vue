    ...
    <div style="margin: 0px 10px ; text-align: left;">
        ...
        :data="state.users.slice((state.currentPage-1)*state.pageSize,state.currentPage*state.pageSize)">
           ...
           <el-table-column label="操作" width="300px">
              <template #default="scope">
                  <el-button type="text" size="small" @click="toSetRole(scope.row.id)">{{ $t('button.wempower') }}</el-button>
                  <el-button  type="text" size="small" @click="resetPw(scope.row.id)">{{ $t('button.wresetPassWord') }}</el-button>
                  <el-button  type="text" size="small" @click="toEditUser(scope.row)">{{ $t('button.wedit') }}</el-button>
                  <el-button    type="text" size="small" @click="deleteUser(scope.row.id)">{{ $t('button.wdelete') }}</el-button>
              </template>
           </el-table-column>
        </el-table>
        <!-- 分页 -->
